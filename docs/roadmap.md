...
**Entregables:** pipeline de importación del dataset de puzzles de Lichess (ADR-0005) con filtrado por rating y generación de posiciones tranquilas; el Radar completo (E5, sin doble solución todavía); Cola Universal con FSRS (E4); calibración muestreada (RF-10.1, 10.2); sesión simple (Cola vencida + Radar); exportación e importación completas (RF-14.1, 14.2).
**Criterio de salida:** un usuario entrena 15 minutos diarios durante una semana solo con Radar+Cola; los fallos reaparecen espaciados; la tasa de acierto converge a 60–80%; una exportación hecha en un dispositivo restaura el estado completo en otro.
